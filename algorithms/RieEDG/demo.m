...
% rate_array = [0.8,0.75,0.65,0.5]
rate_array = [0.4,0.3,0.1]
...
n = 500;
r = 3;
X = rand(n,r);%% Create matrix with pairwise distances
X_centered = X - sum(X,1)/n;
%P = P - sum(P,1)/n; %centering the data
P = X_centered';
...
rate = rate_array(j)
    for k=1:num_trials
        ...
        if descent_mode == 0
            ...
            [D_approx,~,~] = distgeo_rgrad_Pomega(D_hat,samples,n,num_iter,D,rate,1,1,norm_thresh);
        ...
        elseif descent_mode == 2
            ...
            oversampling(i,j,k) = ceil(ceil(L*rate)/deg_freedom)
