...
% --------------------------------------------------------------------------
function [Global_Coordinate, IPM_Recon, output]=alternating_completion(Dist,OmegaMatrix,opts,lsopts)
...
for i = 1:opts.maxit
    ...
    if isfield(opts,'printenergy') && opts.printenergy==1
        if mod(i,100) == 0
...
% -------------------------------------------------------------------------
% if isfield(opts,'printerror') && opts.printerror==1
% fig1=figure(1);
...
[D,IJ] = sort(D,'descend');
V = V(:,IJ);
Global_Coordinate = real(V(:,1:rtilde)*diag(sqrt(D(1:rtilde))));
% -------------------------------------------------------------------------
% output parameters: constraint energy E1, total energy E, relative error 
% in the gram matrix
% -------------------------------------------------------------------------
output.time = time(1:num_it);
