function diff_norm = compute_iterate_diff_norm(Xc,Xc_,meas_op,mode)
...
%     diff_norm = 5*norm(Xc.XcOm-Xc_.XcOm);
% else
    if strcmp(mode,'exact') % this is exact (does not suffer from numerical errors 
        ...
        Xcfull = get_densemat_from_compact(Xc,meas_op);
        Xc_full = get_densemat_from_compact(Xc_,meas_op);
    ...
    elseif strcmp(mode,'efficient') % this is computationally efficient as it 
        ...
        diff_norm = get_frob_diff_compact(Xc,Xc_,meas_op);
