...
r = b - iterapp_mod('mtimes',afun,atype,afcnstr,x,varargin{:});
...
for ii = 1 : maxit
    if existM1
        y = iterapp_mod('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
        if ~all(isfinite(y))
            flag = 2;
            break
        end
    ...
    if existM2
        z = iterapp_mod('mldivide',m2fun,m2type,m2fcnstr,y,varargin{:});
        if ~all(isfinite(z))
            flag = 2;
            break
        end
    ...
    q = iterapp_mod('mtimes',afun,atype,afcnstr,p,varargin{:});
    pq = p' * q;
    if ((pq <= 0) || isinf(pq))
        flag = 4;
        break
...
if (flag == 0)
    ...
    r_comp = b - iterapp_mod('mtimes',afun,atype,afcnstr,xmin,varargin{:});
